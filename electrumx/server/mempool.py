...
class MemPoolAPI(ABC):
    ...
    async def on_mempool(
        ...
        height,
        assets,
        tag_qualifiers_touched,
        tag_h160_touched,
        broadcasts_asset_touched,
        freezes_asset_touched,
        verifier_string_asset_touched,
        restricted_qualifier_touched,
    ):
...
class MemPool(object):
    ...
    def __init__(self, env, api, refresh_secs=5.0, log_status_secs=60.0):
        ...
        self.qualifier_tags: Dict[str, Dict[bytes, Tuple[bytes, int, bool]]] = (
            defaultdict(dict)
        )
        self.tx_to_qualifier_tags: Dict[bytes, Set[str]] = defaultdict(set)

        # h160 -> dict[ txid, (qualifier, tx_pos, flag) ]
        self.h160_tags: Dict[bytes, Dict[bytes, Tuple[str, int, bool]]] = defaultdict(
            dict
        )
        self.tx_to_h160_tags: Dict[bytes, Set[bytes]] = defaultdict(set)

        # asset -> dict [ txid, (data, expr, tx_pos) ]
        self.broadcasts: Dict[str, Dict[bytes, Tuple[bytes, Optional[int], int]]] = (
            defaultdict(dict)
        )
        ...
        self.qualifier_associations: Dict[str, Dict[bytes, Tuple[int, int, str]]] = (
            defaultdict(dict)
        )
    ...
    async def _logging(self, synchronized_event):
        ...
        while True:
            ...
            if self.h160_tags:
                log_msg += (
                    f", {sum(len(x) for x in self.h160_tags.values()):,d} h160s tagged"
                )
            if self.broadcasts:
                log_msg += (
                    f", {sum(len(x) for x in self.broadcasts.values()):,d} broadcasts"
                )
            if self.freezes:
    ...
    def _accept_transactions(
        ...
        for tx_hash, tx in tx_map.items():
            ...
            tx.fee = max(
                0,
                (
                    sum((v if not asset else 0) for _, v, asset in tx.in_pairs)
            ...
            if tx_hash in tx_to_qualifier_associations:
                restricted_qualifier_touched.update(
                    tx_to_qualifier_associations[tx_hash]
                )
    ...
    async def balance_delta(self, hashX, asset) -> Dict[Optional[str], int]:
        ...
        def is_asset_valid(db_asset):
            if asset is None or asset is False:
    ...
    async def get_qualifier_tags(self, asset: str):
        ret = {}
        for txid, (h160, tx_pos, flag) in self.qualifier_tags.get(
            asset, dict()
        ).items():
            ret[h160.hex()] = {
                "flag": flag,
    ...
    async def restricted_verifier(self, asset: str):
        for txid, (qual_pos, res_pos, string) in self.verifiers.get(
            asset, dict()
        ).items():
            return {
                "string": string,
    ...
    async def restricted_assets_associated_with_qualifier(self, asset: str):
        ...
        for txid, (
            ...
            res_pos,
            restricted_asset,
        ) in self.qualifier_associations.get(asset, dict()).items():
