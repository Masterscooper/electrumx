...
- Check [The DogeCore Repo](https://github.com/Masterscooper/DogeGPU/releases) for the latest release of  doge-x.x.x.x-x86\_64-linux-gnu.zip. Download (e.g. through **wget**) and unpack the archive inside the zip – this will probably require installing unzip through **sudo apt install unzip**.

- Create a directory .doge (**mkdir .doge**) in your home directory (e.g., /home/electrumx) and create a file doge.conf inside this directory with the following content (vi, vim, nano etc.): 
...
- Note the txindex=1 above, which is required for the electrumx server. If you already run a node without this setting, add it and perform a restart of doged with the option –reindex. This will take significant time.  

- For better maintenance during updates, you may want to set up a link to your doge binary directory, e.g.: 

**ln –s /home/electrumx/doge-x.x.x.x /home/electrumx/doge**

This way you can always access the binary files through ./doge/bin/... from your home directory. 

- You can now start the doge node with **./doge/bin/doged** . After some time you can check the state of the node using **./doge/bin/doge-cli getblockchaininfo** . Keep an eye on the “blocks” – these will remain at 0 until all headers are synced (can be half an hour depending on the system and connection) and it will ultimately reach the current block-height of the dogpu chain. By this time, which is normal to take several hours, about 20 GB of data will have been downloaded into the working ./.doge directory. 

- At this point you could install some script or service into /etc/systemd/system to run the node automatically at server startup. This ensures a 24/7 operation in case of an unexpected server restart due to system update or maintenance. You could also install watch-dog scripts to restart the server in case of a crash, however, this is beyond the scope of this document. See [This medium article](https://medium.com/@benmorel/creating-a-linux-service-with-systemd-611b5c8b91d6) for an example. It's important to know that a dogegpu node service file requires a type of "forking".

- Although you could continue with installing the electrumx server, it is suggested that you wait until the DogeGPU chain is fully synced to avoid later issues. You will not be able to start the electrumx server without a fully synced chain so some patience is required here.
...
The electrumx server is almost entirely written in python and can be found [HERE](https://github.com/Electrum-DOGPU-SIG/electrumx-dogegpu/releases/).

Full documentation and information about installation / running of the electrumx server can be found [HERE](https://electrumx-dogegpu.readthedocs.io/en/latest/)
...
- Download latest version of electrumx dogegpu with **git clone https://github.com/Masterscooper/electrumx.git** or download from the link above (e.g. wget <path>) – the tar.gz version is suggested as that can easily be unpacked with tar –xzf {name}.tar.gz . 
...
DAEMON\_URL = http://\<rpcuser\>:\<rpcpwd\>@localhost (user/pwd from rpc in doge.conf)
...
- Now double-check that the dogpu node is running and up-to-date with the blocks:**./doge/bin/doge-cli getblockchaininfo** if that looks ok you can start the electrumx server: **systemctl start electrumx** 
